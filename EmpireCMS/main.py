...
def smoke_test(report_name):
    ...
    #生成测试报告并发送邮件
    create_excel_report_and_send_email(excel, ['lihong2023002@126.com'], report_name, "请查收附件：UI自动化测试报告-冒烟测试")

 # 全量测试：执行主sheet的用例集

def suite_test(report_name):
    excel  = main_suite_process(TEST_DATA_FILE_PATH,"测试用例集")
    create_excel_report_and_send_email(excel, ['lihong2023002@126.com'], report_name, "请查收附件：UI自动化测试报告-全量测试")
